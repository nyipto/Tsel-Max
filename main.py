...
def log(value, status='[Inject GameMax]', color='[CC]'):
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            self.log('error sialakan  close proxy tunnel dan qpyton dan mulai ulang'.format(self.inject_host, self.inject_port), color='[R1]')
...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='[P1]INJECT', color='[CC]'):
    ...
    def run(self):
        try:
            ...
            self.log('[R1]status 403 '.format(self.proxy_host, self.proxy_port))
            ...
            self.log('[G1]200 ok''[CC]'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
...
def main():
    print(colors('\n'.join([
     ...
     '[G1] ','[CC]'
     '[G1]       Fb : Nyipto Nanda     ','[CC]'
     '[G1]       ','[CC]'
     '[G1]       Script By Nyipto Nanda   ','[CC]'
     '[P1]       ','[CC]'
     '[R1] HOST : 127.0.0.1 ','[CC]'
     '[R1] PORT : 8080 ','[CC]'
   
     '[G1] - SUNEK LAKUKAN ON/OFF DATA','[CC]'
     '[G1] - ATAU MODE PESAWAT ','[CC]'
     '[G1] - Sabar Biar Nggak Ambyar ','[CC]'
     '[G1] - Config Premium/ ','[CC]'
     '[G1] - Terimakasih Telah Order Kuota ','[CC]'
